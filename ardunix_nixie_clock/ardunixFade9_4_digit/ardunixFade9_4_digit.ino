...
// anode pins
// Note that these are controlled directly on PORT D, we don't use digitalWrite() for these
#define ledPin_a_4  0     // Low  - Mins  units // package pin 2  // PD0
#define ledPin_a_3  1     //      - Mins  tens  // package pin 3  // PD1
#define ledPin_a_2  2     //      - Hours units // package pin 4  // PD2
#define ledPin_a_1  4     // high - Hours tens  // package pin 6  // PD4
...
byte ourIP[DIGIT_COUNT]           = {0, 0, 0, 0}; // set by the WiFi module, if attached
...
void setNextMode(int displayMode) {
  ...
  switch (displayMode) {
    ...
    case MODE_FADE_STEPS_DOWN: {
        loadNumberArrayConfMode(displayMode);
        displayConfigModeWide();
        break;
      }
...
void processCurrentMode(int displayMode) {
  switch (displayMode) {
    case MODE_TIME: {
        ...
        if (tempDisplayModeDuration > 0) {
          ...
          if (tempDisplayMode == TEMP_MODE_VERSION) {
            loadNumberArrayConfIntWide(SOFTWARE_VERSION);
          }
          ...
          if (tempDisplayMode == TEMP_IMPR) {
            loadNumberArrayConfIntWide(lastImpressionsPerSec);
    ...
    case MODE_FADE_STEPS_UP: {
        ...
        loadNumberArrayConfIntWide(fadeSteps);
        displayConfigWide();
    ...
    case MODE_TARGET_HV_UP: {
        ...
        loadNumberArrayConfIntWide(hvTargetVoltage);
        rawHVADCThreshold = getRawHVADCThreshold(hvTargetVoltage);
        displayConfigWide();
        break;
      }
    ...
    case MODE_PULSE_UP: {
        ...
        loadNumberArrayConfIntWide(pwmOn);
        displayConfigWide();
        break;
      }
    case MODE_PULSE_DOWN: {
    ...
    case MODE_MIN_DIM_UP: {
        ...
        loadNumberArrayConfIntWide(minDim);
        displayConfigWide();
        break;
      }
    case MODE_MIN_DIM_DOWN: {
    ...
    case MODE_ANTI_GHOST_UP: {
        ...
        loadNumberArrayConfIntWide(antiGhost);
        displayConfigWide();
        break;
      }
    case MODE_ANTI_GHOST_DOWN: {
...
// ************************************************************
// Break the time into displayable digits, hours and minutes
// ************************************************************
...
// ************************************************************
// Break the date into displayable digits, minutes and seconds
// ************************************************************
...
// ************************************************************
// Break the year into displayable digits
// ************************************************************
...
// ************************************************************
// Show an 1/4 IP configuration value
// ************************************************************
...
// ************************************************************
// Show an integer configuration value, use all 4 digits
// ************************************************************
...
// ************************************************************
// Show an configuration mode value
// ************************************************************
void loadNumberArrayConfMode(byte mode) {
  NumberArray[3] = 0;
  NumberArray[2] = 0;
  NumberArray[1] = (mode / 1) % 10;
  NumberArray[0] = (mode / 10) % 10;
}
...
void digitOn(int digit, int value) {
  switch (digit) {
    case 0: PORTD = PORTD | B00010000; break; // PD4 - equivalent to digitalWrite(ledPin_a_1,HIGH);
    case 1: PORTD = PORTD | B00000100; break; // PD2 - equivalent to digitalWrite(ledPin_a_2,HIGH);
    case 2: PORTD = PORTD | B00000010; break; // PD1 - equivalent to digitalWrite(ledPin_a_3,HIGH);
    case 3: PORTD = PORTD | B00000001; break; // PD0 - equivalent to digitalWrite(ledPin_a_4,HIGH);
  }
