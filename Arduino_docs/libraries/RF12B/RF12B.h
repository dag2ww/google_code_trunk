...
class RF12B {
	public:
		...
		void send(byte * buf, byte length, boolean preventCollision);
		RFPacket recvPacket();	
	...
	private:
		int _mode;
		bool _rfa;
		volatile byte _remaining;
		volatile byte _state;
		volatile bool _packet_received;
		volatile byte _recv_buffer[BUFFER_SIZE];
		volatile byte _r_buf_pos;
		byte _pan_id;
		uint8_t _id;
		float _band;
	protected:
