...
RFPacket::RFPacket(volatile byte buf[],byte size) {
	_rcrc = 0;
	parse(buf,size);
}

RFPacket::RFPacket(volatile byte * buf, byte length, byte id, uint16_t seq, byte type) {
	byte crc = 0;
	_rcrc = 0;
  	rfpacket.p.crc = 0;
...
void RFPacket::parse(volatile byte * buf,byte size) {
	uint8_t crc = 0;
